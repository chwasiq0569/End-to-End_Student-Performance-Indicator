...
class DataTransformationConfig():
    pre_processor_obj_file_path = os.path.join(
        'artifacts', 'data_transformer.pkl')
    ...
    def initiate_data_transformation(self, train_path, test_path):
        try:
            ...
            input_feature_train_df = train_df.drop(
                columns=train_df[target_column_name], axis=1)
            target_feature_train_df = train_df[target_column_name]

            input_feature_test_df = test_df.drop(
                columns=test_df[target_column_name], axis=1)
            target_feature_test_df = test_df[target_column_name]

            logging.info(
                "Applying Preprocessing on training and testing DataFrame...")
